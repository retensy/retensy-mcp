...
- `AI_REPLY` — ответ модели.
  ...
  В этом случае `saveTo` НЕ обновляется — переменная хранит значение прошлого прогона (или
  остаётся пустой, если прогона ещё не было). Веб-редактор поля `quotaFallbackText` не
  показывает — задать его можно только через граф JSON/MCP (`update_graph`/`edit_graph_live`).
...
- Действие **`issue_invoice`** внутри `ACTIONS` — настоящий счёт на **кассу владельца бота**
  ...
  - **Выходы даёт БЛОК `ACTIONS`** (тем же приёмом, что и `error`): при наличии счёта у блока
    ...
    - Ветка `paid` ЕСТЬ → сценарий встаёт на паузу до подтверждения оплаты кассой; не дождался за
      `timeoutMinutes` → уйдёт в `timeout`.
  ...
  - Повторный проход по счёту: пока подписчик ещё ждёт оплату на ЭТОМ шаге — та же ссылка (тот же
    платёж); после оплаты или таймаута — новый счёт. Оплата, пришедшая после таймаута, сценарий не продолжает.
  ...
  - Бот выключен в момент оплаты → ЮKassa повторяет уведомление до суток; `paid` сработает, когда бот
    снова включён. Выключен дольше суток — сценарий по этой оплате не продолжится.
