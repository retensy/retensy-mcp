...
- `AI_REPLY` — ответ модели. Два режима, переключатель — поле `mode` (`"simple"`/не задано, либо
  ...
  **нет выхода `next`** — вместо него именованные ветки ниже.
  ...
  { "mode": "agent", "knowledgeBaseId": "<id базы знаний>", "strict": true,
    ...
    "directAnswer": true, "directAnswerThreshold": 0.88, "minScore": 0.35,
    "topK": 5, "maxContextChars": 6000, "historyTurns": 6,
  ...
  - `knowledgeBaseId` — id базы знаний ЭТОГО владельца (иначе рантайм ведёт себя так, будто базы
    нет). `strict` (по умолчанию `true`) — отвечать только по базе; без базы в строгом режиме
    публикация падает с `AGENT_NO_KNOWLEDGE_BASE`. Без строгого режима модель может отвечать по
    общим знаниям, но не про цены/сроки/обещания — это всегда только из базы и инструкции.
  - `directAnswer` — прямой ответ готовой парой вопрос-ответ БЕЗ обращения к модели (бесплатно),
    когда совпадение по базе ≥ `directAnswerThreshold`. Работает только для документов-пар
    вопрос-ответ (не для кусков файлов/сайта) — кусок PDF, отданный дословно, выглядел бы поломкой.
  ...
  - `systemPrompt` — инструкция агента. Мягкая норма — **до 10 000 символов** (длиннее модель хуже
    держит середину, ответ дороже — только предупреждение при публикации, не блокирует). Жёсткий
    потолок — **30 000 символов** (`AGENT_PROMPT_TOO_LONG`, блокирует). Платформа сама добавляет
    ПЕРЕД инструкцией служебный блок: защиту от инъекций/вытягивания инструкции через реплики
    клиента, историю и фрагменты базы, запрет писать, что действие уже выполнено (это делает
    сценарий, не модель), и формат ответа под конкретный мессенджер — **дублировать это в
    `systemPrompt` не нужно**.
  ...
  - Выходы: `answered` (ответила модель, без строгого режима — либо по базе, либо по общим
    знаниям), `action_<id>` (по числу `actions[]`), `unknown` (не знает ответа — ветка не
    подключена не блокирует публикацию, только предупреждение, но диалог упрётся в тупик),
    `budget_exhausted` (месячный AI-бюджет исчерпан). Прямой ответ (`directAnswer`) отправляется
    и идёт в выход `answered`, как обычный ответ модели.
