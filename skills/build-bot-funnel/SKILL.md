...
3. **Сгенерируй JSON** формата `retensy-bot-graph` (см. схему). Требования к корректности (иначе не опубликуется) — в [reference/validation.md](reference/validation.md). Главное:
   ...
   - `AI_REPLY` («AI-агент») — ответ модели по `systemPrompt`/`userPromptTemplate` (обязателен, иначе выход `error`). Платный узел (`PREMIUM_NODE_FORBIDDEN` на бесплатном тарифе) и недоступен в Instagram-графах (нет в IG-allowlist). Тратит месячный AI-бюджет тарифа — он в токенах, не в рублях. **Исчерпание бюджета — не ошибка**: узел шлёт `quotaFallbackText` (если задан) и идёт дальше по `next`, как обычно; отдельного выхода на этот случай нет. Подробности конфига — в schema.md.
